...
rfm9xR = adafruit_rfm9x.RFM9x(spi, CS, RESET, RADIO_FREQ_MHZ)
rfm9xT = adafruit_rfm9x.RFM9x(spi, CS2, RESET2, RADIO_FREQ_MHZ2)
...
def transmit(counterT):
    while True:
        ...
        time.sleep(2)
        rfm9xT.send(bytes("startup message nr {} from radio T node {} ".format(counterT, rfm9xT.node), "UTF-8"))
        print("Sent package2...")
        counterT = counterT + 1
...
def receive():
    while True:
            ...
            if packet is not None:
                ...
                # after 10 messages send a response to destination_node from my_node
                #time.sleep(0.5)  # brief delay before responding
                #    rfm9x.identifier = counter & 0xFF
                #    rfm9x.send(
                #        bytes(
                #            "message number {} from node {} ".format(counter, rfm9x$                #            "UTF-8",
                #        ),
                #        keep_listening=True,
                #    )
