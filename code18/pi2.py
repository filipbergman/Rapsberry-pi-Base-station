...
def tx(radio, count, size, src, dst):
    ...
    while count:
        result = radio.send(bytes("2222Startup message {} from node {}".format(counter, rfm9x.node), "UTF-8"))
        if not result:
            status.append(False)
        else:
            print("TRANSMITTED")
...
while True:
    rx_process = Process(target=rx, kwargs={'radio':rfm9x2, 'count': counter2, 'src': rfm9x2.destination})
    tx_process = Process(target=tx, kwargs={'radio':rfm9x, 'count': counter, 'size': 250, 'src': rfm9x.node, 'dst': rfm9x.destination})
